...
**Draining is blocked while any index exists (eng-review fix).** Index-shard placement (`indexShardIdForKey`) hashes over the globally *active* shard set, recomputed fresh on every read/write — unlike base-row routing, which uses a persistent vbucket-to-shard mapping unaffected by shard-count changes. Draining a shard changes that active set, which changes the hash modulo for nearly every existing index key, silently orphaning `__cf_indexes` entries with no migration path. `CatalogDO./drain-shard` therefore rejects 409 `SHARD_DRAIN_BLOCKED_BY_INDEXES` if any index is registered cluster-wide (checked via `index_rules`, which is fanned out identically to every catalog shard). Drop all indexes first, drain, then recreate them — backfill will then use the post-drain shard set.
