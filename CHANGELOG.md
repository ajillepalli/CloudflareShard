...
## [Unreleased] — Milestone 3: Topology Service
...
- Migration state machine + dual-write (Chunk 3): `vbucket_map` gains `migration_status`/`target_shard_id`; during a migration the gateway applies writes to the source (authoritative) and mirrors them to the target with the same requestId; mirror failures enqueue on the source's new `__cf_mirror_pending` (alarm-driven retry, backoff 1s→60s, no attempt cap). `CoordinatorDO` mirrors committed `/v1/tx` intents post-commit.
