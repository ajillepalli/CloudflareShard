...
### Security
- Tenant `/v1/sql` writes to ShardDO's internal bookkeeping tables (`__cf_*`, `applied_requests`, `pending_intents`, `row_locks`, `index_pending_jobs`, `sqlite_*`) are rejected 403 `INTERNAL_TABLE_WRITE_FORBIDDEN`. Without this a tenant could lift a cutover fence, forge/delete row provenance, or purge the mirror queue from the data plane — `body.table` (routing only) says nothing about what table the SQL text touches. The guard blocks both the write target (however quoted or `schema.`-qualified) and any bare reference to an internal name, without false-positiving on a double-quoted string value.
