...
async function insertRowBypassingProvenance(shardId: string, table: string, id: string, v: string): Promise<void> {
  const shardStub = env.SHARD.get(env.SHARD.idFromName(shardId));
...
async function rowOwnerEntries(shardId: string, table: string, partitionKey: string) {
  const shardStub = env.SHARD.get(env.SHARD.idFromName(shardId));
