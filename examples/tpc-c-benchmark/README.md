...
9. **Counter updates (stock quantity/YTD, warehouse/district YTD, customer balance/payment/delivery counts) use a compare-and-swap `where` guard.** CloudflareShard's structured mutations only support SETting a column to a literal client-supplied value — there's no server-side arithmetic UPDATE (`col = col - ?`) — so every counter increment/decrement here is computed client-side from a prior read, then written back. Two concurrent transactions reading the same hot row (a popular item's stock, a warehouse's YTD total) before either commits could otherwise silently overwrite each other's effect with a stale value; the `where` clause matches every field read, so a losing write's predicate won't match. New-Order's stock decrement uses this guard inside its per-line `/v1/tx` calls (see item 10) — those calls need atomicity with their paired `order_line` insert, and a dropped update there is compensated afterward, not retried live. **Payment and Delivery's customer-credit updates went further**, after review found that bundling a CAS-guarded update inside `/v1/tx` makes a losing guard invisible: `/v1/tx` reports the whole transaction "committed" with no per-mutation `rowsAffected`, so the benchmark could count a Payment as fully successful even when its warehouse/district/customer updates silently didn't apply. Payment's three updates are now separate `/v1/mutate` calls (which do report `rowsAffected`), each in its own bounded read-compute-write retry loop against a fresh read on a losing guard — a real, deliberate trade-off: Payment no longer commits atomically across all four rows the way real TPC-C Payment specifies (a crash between two of the calls could leave it partially applied), in exchange for the retry being provably safe. A naive alternative — commit the old `/v1/tx` bundle, then re-read afterward and retry the whole transaction if the guarded fields don't match what was intended — was considered and rejected: it can't tell "my update never applied" apart from "my update applied and a later Payment's also applied on top of it," and retrying in the second case would silently apply the same payment amount twice. Delivery's customer credit was converted to the identical retry-loop pattern for the same reason (see item 11).
10. **New-Order's `new_order` marker (what makes an order visible to Delivery) is inserted last, only once every order line has committed and, if any line failed, the already-committed lines and header have been compensated (reversed) first.** Five real bugs were found and fixed here across four rounds of review, each verified live against fresh warehouses under contention: (1) inserting the marker alongside the header, up front, let a concurrent Delivery pass observe an order that was still mid-flight, summing an incomplete set of lines and orphaning whichever hadn't landed yet — fixed by deferring the marker insert to the very end. (2) `runPool`'s own `Promise.all` used to reject on the first line to fail without waiting for other still-in-flight lines, so other lines could keep committing in the background after the function had already thrown past the marker insert — fixed by catching each line's own error inside the pool instead of letting it propagate, so every line is always awaited to completion first. (3) even after both of those fixes, a genuinely failed line still left its already-committed siblings and the header permanently uncompensated — fixed with a real compensating-transaction step (`compensateFailedOrder`): each already-committed line's stock update is reversed via its own compare-and-swap (matching exactly the values that line applied, so it safely no-ops instead of corrupts if a different write touched the row again meanwhile), its order_line row is deleted, and the orphaned `orders` row is deleted (leaving an expected, harmless gap in the o_id sequence — `d_next_o_id` is deliberately not decremented back, since a different, newer order may have already claimed the incremented value). (4) A remote line whose `order_line` insert succeeded but whose stock update then failed threw before it could return its compensation record to the caller, so `compensateFailedOrder` never learned about that already-inserted row — fixed by compensating it immediately, inline, at the point of failure. (5) Two lines within the *same* order referencing the same `(supplyWarehouseId, i_id)` stock row could race their compare-and-swap updates; for a same-warehouse pair, `/v1/tx` still reports "committed" even when the second update's `where` guard silently didn't match (no per-mutation `rowsAffected` from `/v1/tx` to detect it), so the order could end up "successful" with both order_line rows inserted but only one of the two stock decrements actually applied — fixed by deduplicating `(supplyWarehouseId, i_id)` pairs within one order's line generation, preventing the collision outright rather than trying to detect it after the fact. Verified live: a high-concurrency, small-item-pool stress run against fresh warehouses triggered the compensation path repeatedly (multiple orders, several already-committed lines reversed each time); cross-referencing every order/order_line/new_order row server-side afterward found zero orphaned and zero incomplete-line-count orders. Genuinely residual after all five fixes: a third concurrent write landing in the narrow window between a line's update and its own compensation reversal makes that reversal safely no-op rather than fully undo — doubly unlikely, and results in a stranded stock decrement, not silent corruption.
11. **Delivery claims a district's oldest undelivered order via its own checked `/v1/mutate` delete before touching anything else**, so a concurrent Delivery worker that read the same "oldest" order (before the first worker's claim committed) sees `rowsAffected: 0` on its own claim attempt and correctly skips the order as already handled — rather than proceeding to re-credit the customer for an order someone else already delivered. This was a real bug (found by review): the marker's delete used to be bundled into a closing `/v1/tx` alongside the order/customer updates, whose response doesn't report per-mutation `rowsAffected` — a second, stale-read worker's delete would silently no-op there while its customer credit could still land. Two more real gaps followed from that same fix and were closed: (a) once the marker is claimed (deleted), anything failing afterward — a not-found lookup, exhausted contention retries — used to strand the order permanently invisible to every future Delivery pass; the marker is now restored on any such failure so a later pass can retry it from scratch. (b) the customer-credit update itself moved out of a `/v1/tx` bundle into its own `/v1/mutate` call with the same bounded retry-against-a-fresh-read loop Payment uses (see item 9), for the identical reason: a `/v1/tx`-bundled CAS guard can silently lose without either the caller or the benchmark ever finding out.
